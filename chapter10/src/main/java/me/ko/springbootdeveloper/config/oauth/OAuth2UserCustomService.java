package me.ko.springbootdeveloper.config.oauth;
...
public class OAuth2UserCustomService extends DefaultOAuth2UserService {
    ...
    // 위에 saveOrUpdate에 빨간줄 뜨는 메서드 정의
...
/*
    부모 클래스인 DefaultOAuth2UserService에서 제공하는 OAuth 서비스에서 정보를 기반으로
    유저 객체를 만들어주는 loadUser() 메서드 사용 -> 객체를 불러왔다. (override 했다.)
    ...
    saveOrUpdate() 메서드를 통해 사용자가 users 테이블에 존재하면 업데이트, 없으면 저장.(DB에 저장)

    OAuth2 설정 파일을 작성할 예정
        -> OAuth2를 사용한다는게 거의 기본적으로 JWT와 연계가 되는데
           기존 스프링 시큐리티를 구현하면서 작성한 설정이 아니라 추가적인 작업이 필요하다는 의미

        -> 기존의 폼 로그인 방식을 구현하려고 사용한 설정을 다 갈아엎어야 한다.
           WebSecurityConfig.java를 다 주석 처리 한다.
