package me.ko.springbootdeveloper.config;

import lombok.RequiredArgsConstructor;
import me.ko.springbootdeveloper.service.UserDetailService;
...
/*
    1. 스프링 시큐리티의 모든 기능을 사용하지 않게 설정하는 코드
        ...
        h2 데이터를 확인하는 데 사용하는 h2-console 하위 url을 대상으로 .ignoring() 메서ㅡ
    ...
    4. 폼 기반 로그인을 설정
        ...
        defaultSuccessUrl() : 로그인이 완료 되었을 때 이동할 경로르 설정 -> /articles

    5. 로그아웃 설정.
        logoutSuccessUrl() : 로그아웃이 완료되었을 때 이동할 경로를 설정
        invalidHttpSesstion() : 로그아웃 이후에 세션을 전체 삭제할지 여부를 설정 -> 나는 true로 설정

    6. csrf 설정을 비활성화 - CSRF 공격을 방지하기 위햇서 사용하는 건데, 실습 좀 편하게 하려고 비활성화 해둠

    7. 인증 관리자 관련 설정. 사용자 정보를 가져올 서비스르 재정의하거나, 인증 방법, 예를 들어
        LADP, JDBD 기반 인증 등을 설정할 때 사용

    8. 사용자 서비스를 설정
        userDetailService : 사용자 정보를 가져올 서비스를 설정. 이 때 설정하는 서비스 클래스는 반드시
            UserDetailsService를 상속받은 클래스여야 한다.
    ...
    회원 가입 구현
        서비스 메서드 작성할 거다. -> 사용자 정보를 담고 있는 객체를 만들 수 있도록 해야 한다.
