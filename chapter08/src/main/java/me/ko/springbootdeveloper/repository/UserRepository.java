...
/*
    이메일로 사용자를 식별 가능, 즉 email을 username으로 봐도 된다. 따라서 사용자 정보를 가져오기 위해서는
    스프링 시큐리티가 이메일을 전달받아야 한다. 스프링 데이터 JPA는 메서드 규칙에 맞춰서 선언하면
    이름을 분석해 자동으로 쿼리(SQL문)를 생성해 준다.
    ...
    서비스 메서드 코드를 작성할 예정 -. service 패키지에 UserDetailService.java 파일 생성
 */