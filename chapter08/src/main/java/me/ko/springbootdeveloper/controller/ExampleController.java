...
public class ExampleController {
    ...
    public String thymeleafExample(Model model) {       // import org.springframework.ui.Model
        ...
        return "example";       // example.html이라는 뷰를 조회한다.
    }
...
/*
    Model 객체는 뷰, 즉 HTML 쪽으로 값을 넘겨주는 객체이다.
    모델 객체는 따로 생성할 필요 없이 코드처럼 argument로 선언하기만 하면 스프링이 알아서 만들어주므로 편리하게 사용 가능

    "person"이라는 키에 사람 정보를, "today"라는 키에 날짜 정보를 저장.

    thymeleafExample()이라는 메서드는 "example"이라는 리턴값을 가지는데, 얘가 @Controller라는
    애너테이션과 합쳐지면 -> 뷰의 이름이라는 의미가 된다.
    ...
    모델 역할 살펴 보명
        이제 모델 "person", "today" 이렇게 두 키를 가진 데이터가 들어가 있다.
        컨트롤러는 모델을 통해서(앞으로 자주 쓰인다.) 데이터를 설정하고,
        모델은 뷰로 해당 데이터를 전달해서 키에 맞는 데이터를 뷰에서 조회라 수 있게 만들어 준다.

        즉, 모델은 컨트롤러와 뷰의 중간다리 역할을 해준다고 생각하면 된다.
...
컨트롤러(Controller)  ---------------------->        hobbies : ["운동", "독서"]   -------------------> 뷰(View)
   ...
   src/main/resources/templates 디렉토리에 example.html을 만든다.
 */