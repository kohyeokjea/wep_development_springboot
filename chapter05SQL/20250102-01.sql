...
-- 거주 국가가 한국이 아닌 회원만 추출하는 쿼리문을 작성하라
SELECT * FROM users WHERE country != "Korea";
...
-- 2010-12-01부터 2011-01-01까지인 회원 정보를 출력해라

SELECT *
	FROM users
	WHERE created_at BETWEEN "2010-12-01" AND "2011-01-01";

-- WHERE 절을 작성하는데 있어서 컬럼명이 먼저 나오고 =, !=, between등을 적용
...
-- 응용 : users에서 거주 국가가 Korea, USA, UK인 회원 정보를 추출하라.
SELECT *
	FROM users
...
-- 응용 : users에서 거주 국가가 Korea, USA, UK가 아닌 회원 정보를 추출하라.
SELECT *
	FROM users
...
-- LIKE : 해당 전치사 뒤의 작은따음표 / 큰따옴표 내에서는 와일드카드를 사용 가능
-- SQL을 해성하는 컴퓨터가 LIKE 코드를 읽는 순간 와일드카드를 감지하는데
...
-- 거주국가가 a로 끝나는 국가를 모두 출력하는 쿼리
SELECT *
	FROM users
	WHERE country LIKE "%a";

-- 거주국가에 S가 들어가기만 하면 다 출력하는 쿼리
SELECT *
	FROM users
	WHERE country LIKE "%S%";
...
-- A 컬럼의 값이 B이다 일 때 특히 null 일 때 '=' 대신 사용한다.
...
-- 2. products에서 id가 20이하고 price는 30 이상인 제품 정보 출력
...
-- 3. users에서 country가 Korea, Cananda, Belgium도 아닌 회원의
-- 정보를 모두 추출할 것.
-- 단, OR 연산자를 사용하지 않고 출력할 것.
SELECT *
	FROM users
	WHERE country NOT IN ("Korea","Cananda","Belgium");
...
-- 5. orders에서 order_date가 2015-07-01부터 2015-10-31까지
-- 아닌 정보만 출력할 것.
SELECT *	
	FROM orders
	WHERE order_date BETWEEN "2015-07-01" AND "2015-10-31";
-- NOT BETWEEN 순인 것만 알면 된다.


-- ORDER BY
-- 현재까지 WHERE을 이용해서 특정한 조건에 합치하는 데이터들을 조회하는 SQL문에 대해서 학습함

-- 이상의 경우 저장된 순서대로 정렬된 결과만 볼 수 있다. -> id라는 PK값에 따라

-- 이번에는 가져온 데이터를 원하는 순서대로 정렬하는 방법에 관한 것이다.
...
SELECT *
	FROM users
	ORDER BY id DESC;			-- DESC : Descending의 의미로 '내림차순'

-- 이상에서 볼 수 있듯이 ORDER BY는 컬럼을 기준으로 행 데이터를 ASC 혹은 DESC로 
-- 정렬할 때 사용 : 숫자의 경우는 쉽게 알 수 있지만, 문자의 경우는
-- 아스키 코드를 기준으로 한다.
	
-- 예제 : users에서 city를 기준으로 내림차순 하여 전체 정보를 출력하라
SELECT *
	FROM users
	ORDER BY city DESC;

-- 예제 : users에서 created_at를 기준으로 오름차순 하여 전체 정보 출력하라
SELECT *
	FROM users
...
-- 하는데, 첫 번째 컬럼 기준으로 오름차순 정력 해보자.
SELECT username, phone, city, country, id
	FROM users
	ORDER BY 1 ASC;
-- 아까와 같이 ORDER BY 1 ASC로 했지만 정렬이 id가 아닌 username을 기준으로
-- 아스키 코드가 적용되어 정렬 방식이 달라졌음을 확인 가능.
-- 즉, ORDER BY는 데이터 추출이 끝난 후를 기점으로 적용이 된다는 점을 명시해야 한다.
...
-- 이상의 경우 각 컬럼 당 명시적으로 ASC 혹은 DESC를 지정해 줘야 함.
...
SELECT *
	FROM orderdetails
	ORDER BY product_id DESC, quantity ASC;		-- ASC는 적어도 되고 안적어도 된다.
	
-- 여태까지 배운 것을 기준으로 실무에서는 어떤 방식으로 데이터 추출해서 사용하는지 예시
-- 실무에서는 select, where, order by를 사용해 다양한 데이터를 추출하는데,
	
-- 1. 배달 서비스 예시
-- 1) 2023-08-01에 주문한 내역 중 쿠폰 할인이 적용된 내영 추출
SELECT *
	FROM 주문정보
	WHERE 주문일자 = '2023-08-01'
		AND 쿠폰할인금액 > 0;
-- 2) 마포구에서 1인분 배달이 가능한 배달 음시검 추출
SELECT *
	FROM 음식점정보
	WHERE 지역 = "마포구"
		AND 1인분가능여부 = 1;  -- 1을 쓸 경우에는 True의미 / 0은 False 의
