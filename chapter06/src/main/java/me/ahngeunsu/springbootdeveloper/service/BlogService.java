...
public class BlogService {
    ...
    /*
        JPA 지원 메서드인 findAll()을 호출해 Article 테이블에 저장돼 있는 모든 데이터를 조회 -> Test에서 이미 해봄

        응답을 위한 DTO 생성 -> dto package에 -> ArticleResponse.java 파일 생성
     */

    // id로 특정 글 조회
    public Article findById(long id) {          // 이 경우 결과값은 하나밖에 없기 때문에 리턴 타입이 Article이어야만 한다. -> id가 PK라서
    ...
    /*
        트랜젝션 - 데이터베이스에서 데이터를 바꾸기 위한 작업 단위를 의미함.
            ...
            예를 들어 계좌 이체를 할 때 이런 과정을 거친다고 가정한다.
                1) A 계좌에서 출금
                2) B 계좌에 입금
            그런데 1)은 성공했는데, 도중에 2)가 실패하면 어떻게 되나요? 고객 입장에서는 출금은 됐는데,
            입금이 안된 상황이 발생한다.
            이런 상황이 발생하지 않으려면 출금과 입금을 하나의 작업 단위로 묶어서,
            즉 트랜잭션이라는 단위로 묶어서 두 작업을 한 단위로 실행하면 된다.
            만약 중간에 실패한다면 트랜잭션의 처음 상태로 모두 되돌리면 된다.(ROLLBACK)
     */
