...
/*
    ...
    When
        - 토큰 추가 API에 요청을 보낸다. 이때 요청 타입은 JSON이며,
          Given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보낸다.

    Then
        - 응답코드가 201 Created인지 확인하고, 응답으로 온 액세스 토큰이 비어있지 않은지 확인
 */

import com.fasterxml.jackson.databind.ObjectMapper;
import me.ko.springbootdeveloper.config.jwt.JwtFactory;
import me.ko.springbootdeveloper.config.jwt.JwtProperties;
import me.ko.springbootdeveloper.domain.RefreshToken;
import me.ko.springbootdeveloper.domain.User;
import me.ko.springbootdeveloper.dto.CreateAccessTokenRequest;
...
/*
    chapter09
        ...
        Refresh Token : 액세스 토큰과 별개의 토큰. 액세스 토큰이 만료되었을 때
                        ...
                        그 결과값으로 AccessToken을 반환받는 구조로 구현했어다.)

        Filter : 실제로 요청되기 전과 후에 URL 패턴에 맞는 모든 요청을 처리하는 기능 제공
               - 세션 로그인 기준으로는 API 하나하나 마다 요청 전후에 검증이 필요했었다.(차이점)
