...
class TokenProviderTest {
    ...
    void validToken_invalidToken() {
        // given -> 일부러 default가 아니라 실패하는 token을 생성하기 때문에 위의 메서드와 given이 다르다
...
/*
    1. generateToken
        ...
        then : jjwt 라이브러리르 사용하여 토큰을 복호화
            -> 토큰을 만들 때 클레임으로 넣어둔 id 값이 given절에서 만든 유저 ID와 동일한지 확인

    2.
        1) validToken_validToken
        given : jjwt 라이브리를 사용, 토큰 생성. -> 만료 시간은 현재 시간으로 부터
                14일 뒤로 만료되지 않은 토큰으로 생성 -> Default로 만들었다 -> JwtFactory.java
    ...
    3. getAuthentication()
        ...
        then : 반환받은 인증 객체의 유저이름(username)을 가져와서 given절에서 초기화한
               subject값인 user@gmail과 같은 값인지 확인

    4. getUserId()
        given : jjwt 라이브러리를 사용해 토큰 생성. 이때 클레임을 추가하여 키는 "id" 값은 1인 유저 ID를 생성
