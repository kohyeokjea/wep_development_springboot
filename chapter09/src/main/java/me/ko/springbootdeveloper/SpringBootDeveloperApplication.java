    ...
    사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
        ...
        토큰 기반 인증 :
            *토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            사용자의 정보를 담은 세션을 생성하고 저장해서 인증했다. -> 세션 기반 인증
            ...
            토큰 기반 인증의 특징
                1) 무상태성
                    ...
                    토큰 기반 인증에서는 클아언트에서 인증 정보가 담긴 토큰을 생성하고 인증하여
                    서버 입장에서는 클라이언트의 인증 정보를 저장하거나 유지할 필요가 X
                    그래서 완정한 무상태(stateless)로 효율적인 검증 가능
                ...
                2) 확장성
                    ...
                    이를 활요해 다른 서비스에 권한을 공유할 수도 있다.

                3) 무결성
                    토큰 기반 인증 방식은 HMAC(Hash-based Message Authentication) 기법이라고도 하는데,
                    토큰을 발급한 이후에는 토큰 정보를 변경하는 행위를 할 수 없음.
                    만약 토큰을 한 글자라도 변경하면 서버에서는 유효하지 않은 토큰이라고 판단한다.

                JWT(JSON Web Token) -> json형태로 주고 받는다
                    ...
                    JWT 구조 : 온점(.)을 기준으로 헤더(header), 내용(payload), 서명(signature)로 이루어짐.
                        ...
                        2) 내용에는 토큰과 관련된 정보를 담는다. 내용의 한 덩어리를 클레임(claim)
                            이라고 하면, 키-값의 한쌍으로 이루어짐.
                            ...
                            - JWT 예시
                            {
                                "iss": "ko@gmail.com" // 등록된 클레임
                                "iat": 1622370678,
                                "exp": 1622372478,
                                "https://ko.com/jwt_claims/is_admin": true, // 공개 클레임
                                "email": "ko@gmail.com",    // 비공개 클레임
                                "hello": "안녕하세요"
                            }

                            iss, iat, exp는 JWT 자체에서 등록된 클레임
                            URI로 네이밍된 https://ko.com/jwt_claims/is_admin : 공개 클레임
                            등록/공개 클레임 제외 나머지는 전부 비공개 클레임에 해당

                        3) 서명 : 해당 코큰이 조작되었거나 변경되지 않았음을 확인하는 용도로 사용.
